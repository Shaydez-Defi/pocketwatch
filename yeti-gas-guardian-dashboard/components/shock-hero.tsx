...
function ResultsHero({
  ...
  const paidCount = useCountUp(insights.totalPaid, animate, 1600)
  const suiCount = useCountUp(insights.totalSui, animate, 1400)
  const savedCount = useCountUp(insights.saved, animate, 1500)

  return (
    <section className="relative overflow-hidden rounded-3xl border border-destructive/25 bg-gradient-to-br from-destructive/10 via-background to-primary/10 p-6 sm:p-10 animate-results-reveal">
      <div
      ...
      <div className="relative grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center">
        <div className="space-y-4">
          ...
          <p
            className={[
              'font-heading text-5xl font-bold tracking-tight text-destructive sm:text-6xl lg:text-7xl',
...
export function ShockHero({
  ...
  if (!hasRealData || !insights) return <LandingHero />
  return <ResultsHero insights={insights} animate={animate} />
}
  ...
  return (
    <div
      className={[
        ...
        styles[tone],
        animate ? `animate-results-reveal ${delayClass ?? ''}` : '',
      ].join(' ')}
    >
